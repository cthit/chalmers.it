...
export function slackErrors(payload: unknown): string[] {
  ...
  const blocks = (value: unknown, path: string) => {
    ...
    items.forEach((value, i) => {
      const p = `${path}[${i}]`;
      const block = object(value, p);
      switch (block.type) {
        case 'header': {
          const t = object(block.text, `${p}.text`);
          if (t.type !== 'plain_text') fail(p, 'header requires plain_text');
          text(t.text, `${p}.text.text`, 150);
          break;
        }
        case 'section': {
          const t = object(block.text, `${p}.text`);
          if (!['plain_text', 'mrkdwn'].includes(String(t.type)))
            fail(p, 'invalid text type');
          text(t.text, `${p}.text.text`, 3000);
          break;
        }
        case 'divider':
          break;
        case 'image':
          url(block.image_url, `${p}.image_url`);
          text(block.alt_text, `${p}.alt_text`, 2000);
          break;
        case 'rich_text':
          list(block.elements, `${p}.elements`).forEach((el, j) =>
            richElement(el, `${p}.elements[${j}]`)
          );
          break;
        default:
          fail(p, `invalid block type ${String(block.type)}`);
      }
    });
    return items.length;
  };
  ...
  if (count > 50) fail('message', 'more than 50 blocks');
  return errors;
}

export function slackText(payload: unknown): string {
  if (Array.isArray(payload)) return payload.map(slackText).join(' ');
